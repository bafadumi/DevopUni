...
export class UIStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: UIStackProps) {
    ...
    const cloudFrontIdentity = new cloudfront.OriginAccessIdentity(
      this,
      `${stage}DevOpsAssignmentIdentity`
    );

    const hostedZone = route53.HostedZone.fromHostedZoneAttributes(
      this,
      `${stage}DevOpsAssignmentHostedZone`,
      {
        hostedZoneId,
    ...
    const domainCertificate = new acm.DnsValidatedCertificate(
      this,
      `${stage}DevOpsAssignmentCertificate`,
      {
        domainName,
        hostedZone,
      }
    ...
    const assetsBucket = new s3.Bucket(
      this,
      `${stage}DevOpsAssignmentAssetBucket`,
      {
    ...
    const cloudFrontDistribution = new cloudfront.Distribution(
      this,
      `${stage}DevOpsAssignmentDistribution`,
      {
